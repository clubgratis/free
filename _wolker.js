...
function getวเลสConfig(userIDs, hostName) {
	const commonUrlPart = `:443?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=/vlesscf#VLESS TLS`;
	const commonUrlPart1 = `:80?encryption=none&security=none&fp=randomized&type=ws&host=${hostName}&path=/vlesscf#VLESS NTLS`;
...
${วเลสMain}\n
<button class="border border-3 border-success rounded-4 fw-bold" style="width: 180px; height: 30px;" onclick='copyToClipboard("${วเลสMain}")'><i>Copy Vless Tls</i></button>
<br />`;
	}).join('\n');
...
${วเลสSec}\n
<button class="border border-3 border-success rounded-4 fw-bold" style="width: 180px; height: 30px;" onclick='copyToClipboard("${วเลสSec}")'><i>Copy Vless Ntls</i></button>
<br />`;
	}).join('\n');
	// Prepare header string
	const header = `
	<p class="text-center text-light fw-bold mt-2"><a style="height: 30px;">SELAMAT DATANG DICLUB</a></p>
</div>`;
